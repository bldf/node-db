(function() {
     ...
     */
    _T.validate=function(str,type){
        var str = str.replace(/^\s+|\s+$/g,''),as = (type+'').split(','),obj={},le=str.length,info='',re=true,R={
    ...
    /**
     * 验证json是否符合我们想要的数据
     * @param validateObj ; 验证的规则
     * @param tar ， 需要被验证的json对象
     */
    _T.ckJ=function(tar={},validateObj={}){
        var reObj = {re:true} ;
        for(let a in validateObj){
            let re,str = tar[a] || '',ar=validateObj[a].split(','),key=ar[1],t=ar[0];
            re =_T.validate(tar[a] || '',key) ;
            !re.re&&(reObj.re=false,reObj.info=t+re.info);
        }
        return reObj ;
    };
    root._T=_T ;
...
// module.exports = _T;
// console.log(_T.ckJ({pwd:'25534',username:'55'},{pwd:'密码,le5-9',abc:'用户名,fk'}));
// console.log('8888888',_T.validate('45','le5-9'));
